        ...
        <View style={styles.row}>
          {KINDS.map((value) => {
            ...
            return (
              <Pressable
                key={value}
                style={selected ? styles.selectedChip : styles.kindChip}
                onPress={() => onPickKind(value)}
                accessibilityRole="radio"
                accessibilityState={{ selected }}>
                <Text style={selected ? styles.selectedChipText : styles.kindText}>
...
function createStyles(c: Colors) {
  const s = StyleSheet.create({
    ...
    // 화면 배경색을 채움으로 쓰면 다크에서 검은 구멍이 된다. `colors.ts` 참고.
    kindChip: {
      flex: 1,
  ...
  return {
    ...s,
    selectedChip: StyleSheet.flatten([s.kindChip, s.kindChipSelected]),
    selectedChipText: StyleSheet.flatten([s.kindText, s.kindTextSelected]),
  };
}
